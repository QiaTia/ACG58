...
$totalPage = ceil($conn->query("SELECT * FROM  `os` WHERE `cate` like '%$cate%' AND `title` like '%$s%' AND `user_id` = '$user'")->num_rows / $pageSize);
?><!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
...
<body>
	<div class="container-fluid">
		...
		<table class="layui-table">
		    ...
		    <tbody>
<?php
$sql = "SELECT * FROM  `os` WHERE `cate` like '%$cate%' AND `title` like '%$s%' AND `user_id` = '$user' ORDER BY `time` DESC LIMIT ".$limit." , ".$pageSize;
$result = $conn->query($sql);
while($info = $result->fetch_assoc()){
	//code~~
