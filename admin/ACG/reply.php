...
$totalPage = ceil($conn->query("SELECT * FROM  `reply` WHERE `con` like '%$s%'")->num_rows / $pageSize);
?><!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
...
<body>
	<div class="container-fluid">
		...
		<table class="layui-table">
			<colgroup><col width="60"><col width=""><col width="280"><col width="100"><col width="75"><col width="35"><col></colgroup>
		    <thead><tr><td>编号</td><td>回复内容</td><td>回复帖子</td><td>用户</td><td>时间</td><td><i class="layui-icon red">&#xe640;</i></td></tr></thead>
		    <tbody>
<?php
$sql = "SELECT * FROM  `reply` WHERE `con` like '%$s%' ORDER BY `time` DESC LIMIT ".$limit." , ".$pageSize;
...
while($info = $result->fetch_assoc()){
	...
	echo "<tr><td>{$info['id']}</td><td>{$info['con']}</td>
		...
		<td><a href='../../user/?id={$info['conid']}' target='_blank'>".sqlSelect('user','id',$info['userid'])['name']."</a></td>
